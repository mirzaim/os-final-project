...
dwait(struct procstat* ps)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        // Found one.
        ps->cpu_burst = p->proc_stat[RUNNING_T];
        ps->waiting_time = p->proc_stat[WAITING_T];
        ps->turnaround = p->proc_stat[TERMINATE_T] - p->proc_stat[START_T];
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir);
